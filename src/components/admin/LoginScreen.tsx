  ...
  <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-10 text-center">
      ...
      <button 
        onClick={async () => {
          try {
            ...
            adminUI?.showToast(`登录失败: ${e instanceof Error ? e.message : '未知错误'}`, 'error');
          }
        }}
    ...
    <div className="pt-10 flex gap-6 grayscale opacity-50">
      <div className="flex flex-col items-center gap-1">
        ...
        <span className="text-[10px] font-bold">AI 智能</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <Cloud size={16} className="text-blue-500" />
        <span className="text-[10px] font-bold">云端同步</span>
      </div>
      <div className="flex flex-col items-center gap-1">
        <Layers size={16} className="text-indigo-500" />
        <span className="text-[10px] font-bold">层级管理</span>
      </div>
    </div>
  </div>
  );
