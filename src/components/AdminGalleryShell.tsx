      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-10 inset-x-4 sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2 z-[500] flex items-center gap-3 bg-brand-navy px-4 sm:px-6 py-4 rounded-3xl shadow-2xl border border-white/10 animate-in fade-in slide-in-from-bottom-5 duration-300 transition-all">
           ...
           <div className="flex items-center gap-2 sm:gap-3 flex-1 justify-around sm:justify-start overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
             <button onClick={handleGroup} className="w-10 h-10 sm:w-11 sm:h-11 bg-white/10 hover:bg-white/20 rounded-2xl flex items-center justify-center text-white transition-all active:scale-90 border border-white/10 shrink-0" title={t.merge}><Layers size={20} /></button>
             <button onClick={handleBatchEdit} className="w-10 h-10 sm:w-11 sm:h-11 bg-white/10 hover:bg-white/20 rounded-2xl flex items-center justify-center text-white transition-all active:scale-90 border border-white/10 shrink-0" title="统一编辑"><Pencil size={20} /></button>
             <button onClick={handleBatchDelete} className="w-10 h-10 sm:w-11 sm:h-11 bg-red-500/20 hover:bg-red-500/30 rounded-2xl flex items-center justify-center text-red-400 transition-all active:scale-90 border border-red-500/20 shrink-0" title={t.delete}><Trash2 size={20} /></button>
             <button onClick={handleBatchHide} className="w-10 h-10 sm:w-11 sm:h-11 bg-slate-500/20 hover:bg-slate-500/30 rounded-2xl flex items-center justify-center text-slate-300 transition-all active:scale-90 border border-slate-500/20 shrink-0" title="批量隐藏"><EyeOff size={20} /></button>
             <button onClick={handleBatchShare} className="w-10 h-10 sm:w-11 sm:h-11 bg-blue-500/20 hover:bg-blue-500/30 rounded-2xl flex items-center justify-center text-blue-400 transition-all active:scale-90 border border-blue-500/20 shrink-0" title={t.share}><Share2 size={20} /></button>
           </div>
