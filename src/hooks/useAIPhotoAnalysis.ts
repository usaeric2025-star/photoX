...
import { isOk, ErrorFactory } from '@/lib/error/ErrorFactory';
...
export function useAIPhotoAnalysis() {
  ...
  const analyzePhoto = useCallback(async (photo: Photo) => {
    ...
    try {
      ...
      const { analyzePhoto: runAnalysis } = await import('@/services/ai/commands');
      ...
      if (isOk(result)) {
        ...
        const { update: updatePhoto } = await import('@/services/photo/commands');
