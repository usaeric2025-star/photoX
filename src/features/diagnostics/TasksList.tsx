...
import { useGlobalTasks } from '#src/hooks/index.js';
...
function TaskItem({ task }: { task: UnifiedTask }) {
  ...
  return (
    <div className="group bg-slate-50/50 border border-slate-100 rounded-2xl p-4 flex flex-col md:flex-row md:items-center gap-4 hover:bg-white hover:shadow-md transition-all duration-300 animate-fade-right">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-sm font-black text-blue-900 truncate uppercase tracking-tight">
            {task.title}
          </span>
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: '已完成', value: stats.completed, icon: "check-circle", color: 'text-green-500', bg: 'bg-green-50' },
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <Icon name={stat.icon as "history" | "zap" | "check-circle" | "alert-triangle"} size={14} className={stat.color} />
            </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between border-b border-slate-50 pb-4">
            ...
            <Button 
              ...
              onClick={() => refetch()} 
              loading={isPending} 
              className="h-8 px-3 rounded-full text-[10px] font-black uppercase tracking-widest bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 shadow-sm"
            >
              刷新
            </Button>
          </div>
