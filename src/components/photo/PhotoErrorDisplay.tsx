...
export function PhotoErrorDisplay({ error, onRetry }: PhotoErrorDisplayProps) {
  ...
  const diagnosticsText = [
    ...
    `${t.errorCodeLabel}${code || t.none}`,
    `${t.traceIdLabel}${traceId || t.none}`,
    `${t.urlLabel}${typeof window !== 'undefined' ? window.location.href : ''}`,
  ...
  return (
    <motion.div 
      variant="scale"
      transition="easeOut"
      className="flex flex-col items-center justify-center text-center p-8 space-y-7 w-full max-w-sm mx-auto bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100"
    >
      <div className={cn(
      ...
      <div className="flex flex-col w-full gap-3 mt-4">
        ...
        >
          {showDetails ? t.hideDiag : t.showDiag}
        ...
        {showDetails && (
          <motion.div 
            variant="slideDown"
            className="w-full mt-2 group relative bg-slate-900 rounded-xl p-4 text-left font-mono text-[10px] text-slate-300 border border-slate-800 shadow-inner overflow-hidden"
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-2 mb-2">
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
                <span className="font-bold text-slate-500 text-[9px] uppercase tracking-wider">{t.diagnostics}</span>
              </div>
            </div>
            <div className="space-y-2 opacity-80 break-all max-h-32 overflow-y-auto no-scrollbar">
              {code && (
                <div className="flex flex-col">
                  <span className="text-slate-500">{t.errorCodeLabel}</span>
                  <span className="text-slate-300">{code}</span>
                </div>
              )}
              {traceId && (
                <div className="flex flex-col">
                  <span className="text-slate-500">{t.traceIdLabel}</span>
                  <span className="text-slate-300">{traceId}</span>
                </div>
              )}
              <div className="flex flex-col">
                <span className="text-slate-500">{t.errorMsgLabel}</span>
