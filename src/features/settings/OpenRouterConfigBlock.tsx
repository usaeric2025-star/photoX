 ...
 * 
 * 处理 OpenRouter API Key 与模型型号的配置。
 */
    ...
    <div className="space-y-4 p-5 rounded-3xl bg-slate-50/50 border border-slate-100">
      <div className="flex items-center justify-between mb-4">
        <div className="space-y-0.5">
           ...
           <p className="text-[8px] text-brand-navy/40 font-bold tracking-widest">多模型引擎</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {isEditingOpenRouter ? text.common.cancel : text.common.edit}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          {isEditingOpenRouter && (
            ...
            >
              {isSaving === 'openrouter' ? '..' : text.common.save}
            </button>
          )}
        </div>
        
        <div className="space-y-1 mt-2 mb-2">
          <label className="text-[10px] font-bold text-brand-navy/60 block">{text.ai.modelLabel}</label>
          <input
            id="openrouter-model-input"
            type="text"
            placeholder={`例如: ${DEFAULT_AI_MODELS.openrouter}`}
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            {isTesting === 'openrouter' ? '测试中...' : text.ai.test}
          </button>
        </div>
      </div>
    </div>
