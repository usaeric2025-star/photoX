...
export function useLocalStorage<T = string>({
  ...
  useEffect(() => { if (getInitialValueInEffect) setValue(readValue()); }, [getInitialValueInEffect]);
  ...
  const setLocalStorageValue = useCallback((val: T | ((prev: T) => T)) => {
    ...
    } catch (error) { ErrorFactory.handle(error, { context: `useLocalStorage.set:${key}` }); }
  }, [key, serialize]);

  const removeLocalStorageValue = useCallback(() => {
    try { storage.remove(key); setValue(defaultValue); }
    catch (error) { ErrorFactory.handle(error, { context: `useLocalStorage.remove:${key}` }); }
  }, [key, defaultValue]);
...
export function useDebouncedCallback<P extends unknown[], R>(callback: (...args: P) => R, delay: number) {
  ...
  (debouncedFn as any).cancel = () => { if (timeoutRef.current) { clearTimeout(timeoutRef.current); timeoutRef.current = null; } };
