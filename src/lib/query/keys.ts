...
const photos = {
  all: ['photos'] as const,
  list: (filters: FilterOptions) => [...photos.all, 'list', sortObject(filters)] as const,
  detail: (photoId: string) => [...photos.all, 'detail', photoId] as const,
  infinite: (filters: FilterOptions, mode: 'public' | 'admin') => 
    [...photos.all, 'infinite', mode, sortObject(filters)] as const,
  count: (filters: FilterOptions) => [...photos.all, 'count', sortObject(filters)] as const,
};

const groups = {
  all: ['groups'] as const,
  list: (filters: GroupFilterOptions) => [...groups.all, 'list', sortObject(filters)] as const,
