 ...
 */
export function FilterBar({ mode, className, compact = false }: FilterBarProps) {
  ...
  return (
    <div className={cn("flex-shrink-0 bg-white", !compact && "border-b", className)}>
      <div className={cn(
        "flex gap-2.5 items-center px-4 max-w-full",
        compact ? "py-1" : "py-3 sm:px-6"
      )}>
        <div className="flex-1 min-w-0">
          <SearchInput />
        </div>
        
        <div className="flex gap-2 shrink-0 items-center overflow-x-auto hide-scrollbar">
          {!compact && (
            <button 
