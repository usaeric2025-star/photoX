...
import { Photo, Category, Tag } from '#src/types/index.js';
...
export const AIService = {
   ...
   * 重新提取（不重新調用 AI，僅解析緩存）
   */
  reExtract: async (photoId: string): Promise<Record<string, unknown>> => {
    return ErrorFactory.unwrap(
      api.admin.photos["photo-ai-reextract"].$post({
        json: { photoId }
      }),
      'AI Re-extraction Failed'
    );
  },
   ...
   */
  resolveTagNames: async (tagNames: string[], allTags: Tag[]): Promise<Tag[]> => {
   ...
   */
  parseToUpdates: async (result: any, allTags: Tag[], categories: Category[]): Promise<Record<string, unknown>> => {
