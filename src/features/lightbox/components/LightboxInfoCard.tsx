...
export function LightboxInfoCard({ 
  ...
  return (
    <div className="relative z-10 px-4 pb-4 mb-[96px] md:mb-[112px] max-w-2xl mx-auto w-full">
      <div
        className="bg-black/60 backdrop-blur-md rounded-xl p-4 text-white hover:bg-black/70 transition-colors shadow-2xl"
      >
        ...
        {expanded && (
          <>
            <div className="mt-3 pt-3 border-t border-white/10 space-y-1 text-sm text-white/60">
              ...
              {slide.description && <p className="mt-2 text-white/80">{slide.description}</p>}
            </div>
            ...
            {(onEdit || onSetCover || onDelete) && (
              <div className="flex items-center gap-2 pt-3 mt-3 border-t border-white/10 justify-end">
                {onEdit && (
                  <button onClick={() => onEdit(slide.id)} className="p-1.5 rounded-lg bg-blue-600/20 text-blue-400">
                    <Icon name="pencil" className="w-4 h-4" />
                  </button>
                )}
                {onSetCover && (
                  <button onClick={() => onSetCover(slide.id)} className="p-1.5 rounded-lg bg-emerald-600/20 text-emerald-400">
                    <Icon name="check-circle" className="w-4 h-4" />
                  </button>
                )}
                {onDelete && (
                  <button onClick={() => onDelete(slide.id)} className="p-1.5 rounded-lg bg-red-600/20 text-red-400">
