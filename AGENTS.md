...
- **性能与手势控制**: 
  ...
  - **灯箱手势与控件**: 灯箱放大仅保留双指 (Pinch) 缩放手势，严禁使用双击 (Double-tap) 或单击 (Single-tap) 触发放大。右下角控制仅保留纯 Icon 图案按钮 (Icon: `refresh-ccw`) 触发「恢复默认」比例，禁用文字标签按钮以保持极简视野。
...
### 规则 1：路由集中集中化 (`src/router.tsx`)
- 全站路由统一由 `src/router.tsx`（`react-router-dom` v7 `createBrowserRouter`）集中管理。
- 严禁引入 `wouter` 或使用旧有 `RouterOrchestrator` 逻辑。

### 规则 2：Admin 嵌套路由拓展
- Admin 相关页面（如设置、批量编辑、诊断看板等）必须统一作为 `/admin` 路由的 `children` 声明。
- 新增 Admin 子页面，在 `src/router.tsx` 的 `/admin` `children` 数组中配置对应 path 与 Component，严禁散落于组件内部手动条件渲染。

### 规则 3：公开页面拓展
- 新增公开页面或全局路由，直接在 `src/router.tsx` 的顶层 `children` 数组中配置。

### 规则 4：测试验证
- 任何路由改动，必须测试验证：
  1. `/` 公开主页正常访问
  2. `/admin` 后台主页正常访问与 Guard 拦截
  3. `/admin/settings` 及其他 Admin 子路由正常加载
  4. 未匹配路径精准命中 `*` (`NotFoundPage`)
...
## 8. 数据库连接与超时规范 (2026-07 最终锁定 & Timeout Hardening)

### 规则 1：连接超时与查询超时配置 (Hardened)
为配合保活 YML（避免冷启动）并确保高并发请求（如批量上传）下的服务器稳定性，数据库连接与查询超时时间必须严格锁定，**禁止任何业务逻辑层面的动态调整**：
- **`statement_timeout` (查询语句超时)**: 必须设置为 `15000` 毫秒 (15秒)。任何 SQL 语句如果执行超过 15 秒必须强制中断，防止僵尸连接和行/表锁卡死数据库。
- **`connect_timeout` (连接建立超时)**: 必须设置为 `10` 秒。由于项目配有保活工作流，正常连接在 500ms 内即可建立，10秒连接建立时间已留有充足安全余量。当连接异常时应「快速失败」(Fail Fast)。
- **`idle_timeout` (空闲连接释放)**:
  - Serverless 环境下设置为 `5` 秒。
  - Container 容器环境下设置为 `10` 秒。
  - 保证空闲连接能被迅速回收，不占用数据库连接池资源。
- **`keep_alive`**: 必须设置为 `30` 秒，防止防火墙或中间代理（如 Supabase Pooler）静默关闭空闲 TCP 连接。
...
- 严禁将数据库超时、连接断开、建连失败 (如 DrizzleQueryError / PostgresError / timeout / canceling / ENOTFOUND 等) 误判定为 404 返回。这类系统/连通性异常必须原样抛出，以便前端 React Query 可以拦截并执行自动退避重试 (Exponential Backoff Retry)。
- 在 `api/_lib/db/index.ts` 中，必须包含连接探活逻辑，在初始化时通过 `SELECT 1` 验证连通性。

## 9. 反饋職責分類與規範 (2026-07 最終鎖定)

### 規則 1：Sonner (即時反饋)
- **原則**: 所有使用者觸發的顯式操作（如儲存、刪除、設定封面等）必須有即時的 Sonner 反饋。
- **工具**: 統一使用 `src/lib/feedback.ts` 中的 `feedback` 物件。
- **Loading 態**: 對於異步操作，優先使用 `feedback.promise` 提供流暢的「載入中 -> 結果」切換體驗。
- **文案**: 成功文案應簡短（如「保存成功」），錯誤文案應包含上下文或解決建議。

### 規則 2：TaskDrawer (後台任務追蹤)
- **原則**: 僅在「批量操作」或「高耗時背景任務」中使用。
- **場景**: 批量上傳 (> 3 張)、批量 AI 分析、批量刪除/合組。
- **細節**: 必須展示具體的進度 (Progress) 與錯誤詳情。

### 規則 3：兩者共存規範
- 當後台任務完成時，應觸發一個全域的 Sonner 成功通知（如「批量上傳完成」），而具體的失敗詳情保留在 TaskDrawer 中供使用者查閱。
