...
// @ts-ignore
import GroupDetailViewRaw from '@/components/GroupDetailView.tsx?raw';
// @ts-ignore
...
const run = async () => {
    ...
    try {
        ...
        const viewMatch = (GroupDetailViewRaw as string).match(headerPattern);
        if (viewMatch && !viewMatch[0].includes('flex-shrink-0')) {
             return { passed: false, message: `Missing flex-shrink-0 on GroupDetailView header`, durationMs: Date.now() - start };
        }
