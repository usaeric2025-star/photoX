...
export const ISSUE_ACTIONS: Record<string, IssueAction> = {
  deduplicate: {
    name: "自动清理重复照片",
    execute: async () => {
      const data = await ErrorFactory.unwrap<{ count?: number; [key: string]: unknown }>(
        api.admin.maintenance.storage.deduplicate.$post(),
        'Deduplicate failed'
      );
  ...
  orphan_files: {
    name: "找回云端孤兒照片",
    preview: async () => {
      const auditData = await ErrorFactory.unwrap<AuditResponse>(
        api.admin.maintenance.storage.audit.$get(),
        'Audit failed'
      );
      return { 
        affectedCount: auditData.orphans.count, 
        message: `扫描完成：在 R2 发现 ${auditData.orphans.count} 个孤儿文件（未在数据库中记录）`,
        truncated: auditData.orphans.truncated,
        samples: auditData.orphans.samples
      };
    },
    execute: async (onProgress) => {
      ...
      const orphans = auditData.orphans.samples || [];
  ...
  ghost_records: {
    ...
    preview: async () => {
      ...
      return { 
        affectedCount: auditData.ghosts.count, 
        message: `掃描完成：發現 ${auditData.ghosts.count} 條殘餘記錄（資料庫中有記錄但 R2 找不到文件）`,
        samples: auditData.ghosts.samples
      };
    },
    execute: async () => {
      const data = await ErrorFactory.unwrap<{ count?: number }>(
        api.admin.maintenance.storage['clean-ghosts'].$post(),
        'Clean ghosts failed'
      );
  ...
  cleanup: {
    ...
    execute: async () => {
      await ErrorFactory.unwrap<unknown>(
        api.admin.maintenance['daily-cleanup'].$post(),
        'Cleanup failed'
      );
  ...
  refresh_view: {
    ...
    execute: async () => {
      await ErrorFactory.unwrap<unknown>(
        api.admin.maintenance['refresh-view'].$post(),
        'Refresh view failed'
      );
  ...
  repair_integrity: {
    name: "修复数据库约束与合组一致性",
    execute: async () => {
      const data = await ErrorFactory.unwrap<{ dissolved?: number; synced?: number }>(
        api.groups['repair-integrity'].$post(),
        'Repair integrity failed'
      );
      return { message: `修复完成：解散了 ${data.dissolved || 0} 个无效合组，同步了 ${data.synced || 0} 个合组。`, data };
    }
  }
};
