...
2. 尺寸（Dimensions）：
   - 仔細從照片或標籤中尋找尺寸信息。
   - 【極為重要】有些照片可能包含多個尺寸（例如總尺寸、座位尺寸、靠枕尺寸，或一套商品中不同組件的尺寸）。請務必提取並列出所有的尺寸！為每一組獨立的尺寸單獨建立一個物件，並準確填寫對應的 "label"（規格/組件名稱，例如：'Overall', 'Chair', 'Table', 'Seat' 等）。
   - 【智能判斷與估算】：如果照片中有明確標示尺寸，請提取它們。若照片中完全沒有標示尺寸，請你根據生活經驗、物品的比例與常見規格，**自行判斷並估算**出該商品的合理尺寸。
   - 【單位判斷】：若自行估算，請使用 'cm' 為單位。若是從照片提取且無明確標示，根據數值判斷（高達三四位數如 450, 800 可能是 'mm'，兩位數如 40, 80 可能是 'cm'，有雙引號 '"' 或 'in' 則是 'inch'）。
   - 每組尺寸必須包含 "label"（必填，請用適當的名稱描述這個尺寸）、"length"、"width"、"height"（若僅有 1 或 2 個維度也請填入對應欄位，沒有的維度填 0）、"unit": "單位" (根據上述判斷填入 'cm', 'mm' 或 'inch')、"isAI": true
