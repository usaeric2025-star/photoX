...
export function PublicHeader({ totalCount, onRefresh, isRefreshing, className }: PublicHeaderProps) {
  ...
  const isAdmin = user?.role === 'admin' || (user && !isStaff && hasAdminAccess);

  const theme = {
    public: {
      bg: "bg-white border-b border-slate-200 text-slate-800 shadow-xs",
      logoColor: "bg-slate-900",
      logoText: "text-slate-900 font-bold",
      button: "bg-white hover:bg-slate-100 text-slate-600 border-slate-200 hover:text-slate-900 transition-all outline-none rounded-lg flex items-center justify-center border shadow-xs",
      badge: "bg-white border-slate-200 text-slate-600",
      badgeLabel: "text-slate-500",
      badgeVal: "text-slate-900 font-bold",
      popoverTrigger: "bg-white hover:bg-slate-100 border-slate-200 text-slate-600 hover:text-slate-900 border transition-all outline-none rounded-lg flex items-center justify-center shadow-xs"
    }
  }['public'];

  return (
    <header className={cn("h-14 shrink-0 px-4 flex items-center justify-between relative", theme.bg, className)}>
      {/* 左侧：Logo & 计数 */}
      <div className="flex items-center gap-2 sm:gap-3 shrink-0 flex-nowrap">
        ...
        {totalCount !== undefined && (
          <div className={cn("hidden sm:flex items-center gap-1.5 text-[10px] font-medium border rounded-full px-2 py-0.5 select-none shrink-0 cursor-default justify-center shadow-sm whitespace-nowrap", theme.badge)}>
      ...
      <div className="flex items-center gap-2 flex-nowrap shrink-0">
        ...
        <NativePopover
          align="end"
          trigger={
            <button type="button" className={cn("w-10 h-10", theme.popoverTrigger)}>
          ...
          <div className="flex flex-col min-w-[220px] p-1 gap-1">
            {hasAdminAccess ? (
              <div className="px-3 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 select-none">
                ...
                <span className="truncate">{user?.email?.split("@")[0] || (isStaff ? 'Staff' : 'Admin')}</span>
              </div>
            ) : (
